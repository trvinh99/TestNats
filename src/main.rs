...
fn start_pipeline(root_path: String) -> Result<(), anyhow::Error> {
    ...
    let pipeline = gst::parse_launch(
        &format!("rtspsrc location=rtsp://10.50.13.252/1/h264major ! rtph264depay !  vaapih264dec ! videoconvert !  x264enc ! mpegtsmux ! multifilesink max-files=5 post-messages=true next-file=2 location={}/hls/ch%05d.ts", root_path)
